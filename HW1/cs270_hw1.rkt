...
(define (sum3 n)
  (if (<= n 1) -1 (+ (sum3 (- n 1))  (* (expt n 2) (expt -1 n))))
)
...
(define (puzzle n)
  (if (<= n 3) n  (+ (puzzle (- n 1)) (puzzle (- n 2))))
)
...
You must write a recursive function.
Hint: the built-in remainder function is helpful to pick off the individual digits.
|#
