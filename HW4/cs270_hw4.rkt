...
(define (int_to_bool_h n L)
  (cond
    [(= n 0)L]
    [(= (remainder n 2) 1) (append (int_to_bool_h (quotient n 2) L) (list #t))]
    [(= (remainder n 2) 0) (append (int_to_bool_h (quotient n 2) L) (list #f))]
))
...
(define (pad num_bits bit_list)
  (cond
    [(= (length bit_list) num_bits) bit_list]
    [else (pad num_bits (cons #f bit_list))]
    )
)
...
(define (tautology result_list)
  (if (equal? (empty? result_list) #t)
      #t
      (if (equal? (first result_list) #f)
          #f
          (tautology (rest result_list))))
)
;Test your function
...
(define (is_taut func n)
  (tautology (evaluate_tt func (tt_inputs n))
      )
)
