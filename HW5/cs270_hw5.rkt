...
(define (lookup target environment)
  (cond 
    ...
    [(eq? (car (first environment)) target) (car (cdr (first environment)))]
...
(define (get-variables expression)
  (cond 
    [(or (or (null? expression) (is-reserved-word? expression)) (is-constant? expression)) '()]
    [(is-variable? expression) (list expression)]	
    [(append (remove-duplicates (get-variables (first expression))) (remove-duplicates (get-variables (rest expression))))]
  )
);end defunc
...
(define (make_bindings name)
  (cons (cons (cons name (cons #t '() )) '() )
        (cons (cons (cons name (cons #f '() )) '() ) '() ))
)
...
(define (insert_binding binding environments)
  (if (cons? environments)
    (cons(append binding (first environments))(insert_binding binding (rest environments)))environments))
...
(define (run-on-truth-table expression tt)
  (cond
    [(null? tt) '()]
    [(append (list (bool-eval expression (first tt))) (run-on-truth-table expression (rest tt)))]
    )
  )
;Tests
...
(define (atleast-one-true my_list)
  (cond
    [(null? my_list) #f]
    [(first my_list) #t]
    [else (atleast-one-true (rest my_list))]
    )
)
